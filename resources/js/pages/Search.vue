...
<section :class="[(showSidebar == true && width > 1180) ? 'pl-[22rem]' : (showSidebar == false || (showSidebar == true && width < 1180)) ? 'pl-[2rem]' : '', 'pt-[2rem] pr-[1rem] bg-background min-h-[calc(127.5vh-20rem)] [@media(max-width:550px)]:pl-[.5rem] [@media(max-width:550px)]:pr-[.5rem]']">
        <h1 class="text-[1.5rem] text-text_dark capitalize">Searching Results</h1>
...
export default {
    ...
    computed: {
        ...
        showSearchPlaylist: function (){
            return store.getters.getSearchPlaylist
        }
    },
    mounted(){
        this.getPlaylists()
    ...
    methods: {
        async getPlaylists(){
            this.info =  this.showSearchPlaylist
            if(this.info){
                let data = new FormData()
                data.append('name', this.info)
