<template>
    ...
    <section :class="[(showSidebar == true && width > 1180) ? 'pl-[22rem]' : (showSidebar == false || (showSidebar == true && width < 1180)) ? 'pl-[2rem]' : '', 'pt-[2rem] pr-[1rem] bg-background min-h-[calc(127.5vh-20rem)] [@media(max-width:550px)]:pl-[.5rem] [@media(max-width:550px)]:pr-[.5rem]']">
        <h1 class="text-[1.5rem] text-text_dark capitalize [@media(max-width:550px)]:text-[1.2rem]">Bookmarked Playlists</h1>
        ...
        <div class="flex gap-[1rem] justify-start items-start pr-[1rem] [@media(max-width:550px)]:flex-col [@media(max-width:550px)]:pr-0">
            <div v-for="(playlist, index) in playlists" :key="index" class="bg-base rounded-lg p-[2rem] w-[33rem]">
                ...
                <div class="flex justify-center gap-[1rem]">
                    <button @click="this.$router.push('/playlist/' + playlist.id)" class="bg-button text-base text-center border-2 border-button rounded-lg py-[.5rem] block w-full transition ease-linear duration-200 hover:transition hover:ease-linear hover:duration-200 hover:text-button hover:bg-base [@media(max-width:550px)]:text-[.8rem] [@media(max-width:550px)]:py-[.2rem] [@media(max-width:550px)]:w-[7rem]">View Playlist</button>
                    <button @click="deleteBookmark(index)" class="bg-button4 text-base text-center border-2 border-button4 rounded-lg py-[.5rem] block w-full transition ease-linear duration-200 hover:transition hover:ease-linear hover:duration-200 hover:text-button4 hover:bg-base [@media(max-width:550px)]:text-[.8rem] [@media(max-width:550px)]:py-[.2rem] [@media(max-width:550px)]:w-[7rem]">Remove</button>
                </div>
            </div>
        </div>
    </section>
    ...
    methods: {
        ...
        async getPlaylists() {
            await this.getUser().then((value) => {
                axios.get('/api/bookmarks/user/' + value.id).then((response) => {
                    this.playlists = response.data.playlists
                    this.playlists.forEach((content) => {
        ...
        deleteBookmark(id) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete('/api/bookmarks/delete/' + this.bookmarks[id].id)
                    Swal.fire({
                    title: "Removed!",
                    ...
                    if(result.isConfirmed){
                        this.$router.push('/bookmarks').then(() =>{this.$router.go(0)})
                    }
                }));
            }
            });
