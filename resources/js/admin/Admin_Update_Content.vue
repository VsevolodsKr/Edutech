    ...
    <section :class="[(showSidebar == true && width > 1180) ? 'pl-[22rem]' : (showSidebar == false || (showSidebar == true && width < 1180)) ? 'pl-[2rem]' : '', 'pt-[2rem] pr-[1rem] bg-background [@media(max-width:550px)]:pl-[.5rem] [@media(max-width:550px)]:pr-[.5rem]']">
            ...
            <div class="flex items-center justify-center">
                <form method="post" enctype="multipart/form-data" class="bg-base rounded-lg p-[1rem] w-[50rem]">
                    ...
                    <p class="text-[1.2rem] text-text_dark [@media(max-width:550px)]:text-[.9rem]">Video status <span class="text-button4">*</span></p>
                    <select v-model="status" name="status" class="text-[1rem] text-text_dark rounded-lg p-[.5rem] my-[1rem] h-[3rem] bg-background w-full outline-none focus:outline-none [@media(max-width:550px)]:text-[.7rem] [@media(max-width:550px)]:w-full" required>
                        <option value="" selected>Select status...</option>
                        <option value="active">active</option>
                        <option value="deactive">deactive</option>
                    </select>
                    <p class="text-[1.2rem] text-text_dark [@media(max-width:550px)]:text-[.9rem]">Video title <span class="text-button4">*</span></p>
                    <input id="title" :value="content.title" type="name" name="text" placeholder="Enter content title..." required maxlength="50" class="text-[1rem] text-text_dark rounded-lg p-[.5rem] my-[1rem] h-[3rem] bg-background w-full outline-none focus:outline-none [@media(max-width:550px)]:text-[.7rem]">
                    <p class="text-[1.2rem] text-text_dark [@media(max-width:550px)]:text-[.9rem]">Video description <span class="text-button4">*</span></p>
                    <textarea id="description" :value="content.description" placeholder="Enter playlist description..." required maxlength="1000" cols="30" rows="10" class="h-[20rem] resize-none w-full rounded-lg bg-background my-[1rem] p-[.5rem] text-[1rem] text-text_dark outline-none focus:outline-none [@media(max-width:550px)]:text-[.7rem] [@media(max-width:550px)]:mb-0"></textarea>
                    <p class="text-[1.2rem] text-text_dark [@media(max-width:550px)]:text-[.9rem]">Video playlist <span class="text-button4">*</span></p>
                    <select v-model="playlist" name="playlist" class="text-[1rem] text-text_light rounded-lg p-[.5rem] my-[1rem] h-[3rem] bg-background w-full outline-none focus:outline-none [@media(max-width:550px)]:text-[.7rem] [@media(max-width:550px)]:w-full" required>
                        <option value="" selected>Select playlist...</option>
                        <option v-for="(playlist, index) in playlists" :key="index" :value="playlist.id">{{ playlist.title }}</option>
                    </select>
                    <p class="text-[1.2rem] text-text_dark [@media(max-width:550px)]:text-[.9rem]">Video thumbnail <span class="text-button4">*</span></p>
                    <div class="relative">
                    ...
                    <p class="text-[1.2rem] text-text_dark [@media(max-width:550px)]:text-[.9rem]">Select video <span class="text-button4">*</span></p>
                    <div class="relative">
                        <video class="w-full h-[20rem] object-cover rounded-lg z-[-120] [@media(max-width:550px)]:h-[12rem]" controls>
                            <source :src="content.video">
                        </video>
                    </div>
                    <input id="video" type="file" accept="video/*" required class="text-[1rem] text-text_light rounded-lg p-[.5rem] bg-background w-full outline-none focus:outline-none my-[1rem] h-[3rem] [@media(max-width:550px)]:text-[.7rem]">
                    <button type="submit" @click="handleSubmit" class="bg-button text-base text-center border-2 border-button rounded-lg py-[.5rem] block w-full transition ease-linear duration-200 hover:transition hover:ease-linear hover:duration-200 hover:text-button hover:bg-base [@media(max-width:550px)]:text-[.8rem] [@media(max-width:550px)]:py-[.2rem]">Upload Content</button>
                </form>
            </div>
        </section>
    <Admin_Sidebar />
...
export default {
    ...
    methods: {
        async handleSubmit(e){
            ...
            data.append('playlist_id', this.playlist)
            ...
            if(document.querySelector('#img').files[0]){
                data.append('thumb', document.querySelector('#img').files[0])
            }
            if(document.querySelector('#video').files[0]){
                data.append('video', document.querySelector('#video').files[0])
            }
            try{
                const response = await axios.post('api/contents/update/' + this.$route.params.id + '/send', data, config)
