<template>
        ...
        <div class="bg-base rounded-lg p-[1rem] w-[50rem]">
            ...
            <div class="flex gap-[2rem]">
                <div class="flex-[1_1_25rem]">
                    ...
                    <input v-model="name" type="name" name="name" :placeholder="teacher.name" required maxlength="50" class="text-[1rem] text-text_light rounded-lg p-[.5rem] bg-background w-full outline-none focus:outline-none [@media(max-width:550px)]:text-[.7rem] [@media(max-width:550px)]:w-full">
                ...
                <div class="flex-[1_1_25rem] [@media(max-width:550px)]:flex-col">
                    ...
                    <input v-model="p_password" type="password" name="password" placeholder="Enter your old password..." maxlength="50" class="text-[1rem] text-text_light rounded-lg p-[.5rem] bg-background w-full outline-none focus:outline-none [@media(max-width:550px)]:text-[.7rem]">        
                    <p class="text-[1.2rem] text-text_light pt-[1rem] [@media(max-width:550px)]:text-[.9rem]">New password</p>
                    <input v-model="n_password" type="password" name="password" placeholder="Enter your new password..." maxlength="50" class="text-[1rem] text-text_light rounded-lg p-[.5rem] bg-background w-full outline-none focus:outline-none [@media(max-width:550px)]:text-[.7rem]">        
                    <p class="text-[1.2rem] text-text_light pt-[1rem] [@media(max-width:550px)]:text-[.9rem]">Confirm password</p>
                    <input v-model="c_password" type="password" name="password2" placeholder="Confirm your new password..." maxlength="50" class="text-[1rem] text-text_light rounded-lg p-[.5rem] bg-background w-full outline-none focus:outline-none [@media(max-width:550px)]:text-[.7rem]">
                </div>
            </div>
    ...
    export default {
        ...
        async handleSubmit(e){
            ...
            data.append('old_email', this.teacher.email)
            ...
            try{
                const response = await axios.post('api/admin_update/send', data, config)
                console.log(response.data.message)
                this.errorList = response.data.message
                this.status = response.data.status
                setTimeout(() => {
                    this.$router.push('/dashboard', this.teacher)
                }, 500)            
            }catch(err){
